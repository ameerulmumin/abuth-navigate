...
import 'authenticate1_model.dart';
export 'authenticate1_model.dart';

class Authenticate1Widget extends StatefulWidget {
  const Authenticate1Widget({Key? key}) : super(key: key);
...
class _Authenticate1WidgetState extends State<Authenticate1Widget>
  ...
  late Authenticate1Model _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => Authenticate1Model());
  ...
  Future<void> checkAndAddUserToVerifiedUsers(BaseAuthUser currentUser) async {
    final FirebaseFirestore firestore = FirebaseFirestore.instance;
    final CollectionReference verifiedUsersCollection =
        firestore.collection("REGISTERED_USERS");

    // Check if the user already exists in the collection
    final DocumentSnapshot userSnapshot =
        await verifiedUsersCollection.doc(currentUser.uid).get();

    if (!userSnapshot.exists) {
      // If the user doesn't exist, add them to the collection
      verifiedUsersCollection.doc(currentUser.uid).set({
        'id': currentUser.uid,
        'email': currentUser.email,
        'name': currentUser.displayName == null ? '' : currentUser.displayName,
        'photoUrl': currentUser.photoUrl == null ? '' : currentUser.photoUrl,
        'status': 'normal', // You can set the initial status here
      });
    }
  }
                                      ...
                                      Padding(
                                        ...
                                        child: FFButtonWidget(
                                          onPressed: () async {
                                            ...
                                            checkAndAddUserToVerifiedUsers(
                                                user);
